...
COORDINATOR_CLOUD_ENV = "RESEARCH_COORDINATOR_CLOUD"
...
async def server_brain(server: Server, *, timeout_s: float = 45.0) -> str:
    ...
    inherit the runner's own RESEARCH_MODEL_MODE -- which is what it used to
...
async def run_matrix(
    ...
    *,
    ...
    return MatrixReport(
        request_summary=f"{request.topic[:60]} (<={request.max_words}w)",
        ...
        model_mode=os.getenv("RESEARCH_MODEL_MODE", "direct"),
        cells=cells,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # A fixed default topic, not a memorable one: the matrix measures the wire,
    # and every cell asks for the same brief so a latency difference between
    # cells cannot be a difference in what was asked.
    parser.add_argument("--topic", default="the A2A protocol and why agents need one")
