...
class Cell(BaseModel):
    ...
    #: RESEARCH_MODEL_MODE produced a table that said direct while measuring
    #: three llm agents.
    server_brain: str = "unknown"
    #: The model named in the draft's serving header, so a latency figure can
    #: be read against the thing that produced it.
    server_model: str = "unknown"
    ok: bool
    #: FailureKind value, or "sdk-missing" when the client SDK is not installed.
    failure_kind: str | None = None
    detail: str | None = None
    latency_ms: float | None = None
    #: Length of the returned draft. Not a quality signal -- it is here because
    #: a cell that "succeeds" with 30 words is worth seeing in the table.
    words: int = 0
    title: str | None = None
