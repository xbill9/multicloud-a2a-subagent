---
title: "Strands on AgentCore, Answering Two Other Clouds: The Contract, the Header, and the microVM"
published: false
description: A Strands agent on Bedrock AgentCore Runtime serving A2A to callers on Google Cloud and Azure. The AgentCore container contract, the request header it drops, why a session is a cold start, least privilege that actually works, and reaching Bedrock from another cloud with no access key.
