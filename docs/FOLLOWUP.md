...
> **Context, 2026-08-12.** Written against the currency mesh. The repo now
> carries three research agents and a judge. The structural criticisms below --
> that the client axis is one transport with three facades, and the server axis
> two stacks with three executors -- are unchanged and still true. Item 5
> ("make deployed consensus non-vacuous") is superseded: consensus is gone, and
> its replacement has its own version of the same problem, which is that
> `direct` mode makes all three drafts identical. See the README.
