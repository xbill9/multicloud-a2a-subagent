...
build() {
  ...
  gcloud run deploy "$MASTER" \
    ...
    --set-env-vars "${public_env},RESEARCH_MAX_CONCURRENT_RUNS=${RESEARCH_MAX_CONCURRENT_RUNS:-2},RESEARCH_MAX_RUNS_PER_HOUR=${RESEARCH_MAX_RUNS_PER_HOUR:-30},RESEARCH_COORDINATOR_CLOUD=gcp,RESEARCH_CLOUD=gcp,OTEL_TRACES_EXPORTER=${OTEL_TRACES_EXPORTER},RESEARCH_JUDGE_MODE=${JUDGE_MODE},RESEARCH_TIMEOUT_SECONDS=${RESEARCH_TIMEOUT_SECONDS},RESEARCH_EVAL_STORE=${EVAL_MOUNT}/runs.jsonl,RESEARCH_FEEDBACK_STORE=${EVAL_MOUNT}/feedback.jsonl,GOOGLE_GENAI_USE_VERTEXAI=true,GOOGLE_CLOUD_PROJECT=${PROJECT},GOOGLE_CLOUD_LOCATION=${REGION}" \
