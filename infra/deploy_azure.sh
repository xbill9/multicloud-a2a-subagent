...
deploy() {
  ...
  if az containerapp show -n "$APP" -g "$RG" -o none 2>/dev/null; then
    ...
    az containerapp create -n "$APP" -g "$RG" \
      ...
      --env-vars "RESEARCH_MODEL_MODE=${MODEL_MODE}" HOST=0.0.0.0 PORT=8080 -o none
  fi
...
foundry() {
  ...
  echo
  echo "the agent still serves RESEARCH_MODEL_MODE=direct; switch it with:"
  echo "  az containerapp update -n ${APP} -g ${RG} --set-env-vars RESEARCH_MODEL_MODE=llm"
}
