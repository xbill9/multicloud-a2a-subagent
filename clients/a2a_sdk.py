...
def _task_texts(task) -> list[str]:
    """Pull the agent's reply out of a completed Task, once.
    ...
    **And then deduplicate, which is the second half of the same finding.**
    ADK puts the reply in the artifact *and* in history, so reading every
    carrier returns it twice. Against the currency agent that was invisible:
    the parser indexed quotes by target currency, so a duplicate JSON object
    overwrote its twin and the answer was right. Against a research draft it
    is not invisible at all -- the body doubles, the word count doubles, and
    `concision` scores a compliant draft as a 100% overrun. One reply carried
    in two envelopes is one reply.
    """
...
class A2ASdkClient(A2AResearchClient):
    stack = "a2a-sdk"
